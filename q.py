...
user_proxy = UserProxyAgent(
    ...
    code_execution_config={"last_n_messages": 3, "work_dir": "./tmp"},
    human_input_mode="NEVER",
)
coder = AssistantAgent(name="Coder", llm_config=llm_config)
critic = AssistantAgent(
    name="Critic",
    system_message="Critic agent's system message here...",
...
def display_help():
    ...
    print(
        "  'feedback [your feedback]': 🧠 Provide feedback using Q-learning to improve responses."
    )
...
def quantify_feedback(critic_feedback):
    ...
    if any(
        keyword in critic_feedback.lower() for keyword in positive_feedback_keywords
    ):
